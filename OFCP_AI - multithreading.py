...
def pool_function(x):
    ...
    current_milli_time = lambda: int(round(time.time() * 1000))
    xtime = current_milli_time()
    count = 0
    predicted_score = 0
    while ( (current_milli_time() - xtime) < 2000 ):
        predicted_score += simulateGame(game_state, row, card) # get expected value for random simulated game with card placed in that row
        count += 1
...
def chooseMove(game_state, card, iterations_timer):
    ...
    #### calculate 1 card placement ####
        ...
        t = p.map(pool_function, [(i, game_state, card) for i in [1, 2, 3]])
        predicted_scores[0][1] = t[0]
        predicted_scores[1][1] = t[1]
        predicted_scores[2][1] = t[2]
        
        p.close() # avoid teh mem0ry l33k
