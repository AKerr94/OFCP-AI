...
function handlePlacements(resp) {
    ...
    // handle AI placements on frontend
    if ('properties2' in gs) {
    ...
    if (cardsPlacedCount < 13 && 'cardtoplace' in gs) { // display player's next card
...
function POST_reqwest(thenFunction) {
    ...
    reqwest({
            'url': 'https://alastairkerr.co.uk/ofc/api/ofc-backend/',
            'method': 'post',
            'data': {
                "game-id": gameId,
